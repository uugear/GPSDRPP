...
namespace backend {
    ...
    int init(std::string resDir) {
        ...
        // Load app icon
        if (!std::filesystem::is_regular_file(resDir + "/icons/gpsdrpp.png")) {
            flog::error("Icon file '{0}' doesn't exist!", resDir + "/icons/gpsdrpp.png");
            return 1;
        }

        GLFWimage icons[10];
        icons[0].pixels = stbi_load(((std::string)(resDir + "/icons/gpsdrpp.png")).c_str(), &icons[0].width, &icons[0].height, 0, 4);
	...
	void minimizeWindow() {
		...
		flog::info("Application window is minimized");
	}
	
	void quitApplication() {
		glfwSetWindowShouldClose(window, GLFW_TRUE);
		flog::info("Quiting the application...");
	}
}
