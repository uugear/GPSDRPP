...
private:
    ...
    public:
        struct TileCacheMetadata {
            std::time_t fetchedAt = 0;
            std::time_t expiryAt = 0;
            std::string etag;
    ...
    private:
        ...
        static std::queue<TileJob> tileJobs;
        static std::vector<std::thread> workers;
        static bool workersStarted;
        static bool stopWorkers;
        ...
        std::string getTileFilePath(const std::string& outputFile) const;
        std::string getMetadataFilePath(const std::string& outputFile) const;
        bool loadTileMetadata(const std::string& metadataFilePath, TileCacheMetadata& metadata);
        bool saveTileMetadata(const std::string& metadataFilePath, const TileCacheMetadata& metadata);
        bool isTileFresh(const std::string& tileFilePath, const std::string& metadataFilePath, TileCacheMetadata* metadata = nullptr);
        bool downloadTile(const TileJob& job);
        void queueTileDownload(const TileJob& job);
