...
#define FREQ_SEL_DIGIT_HEIGHT       80
#define FREQ_SEL_DIGIT_HALF_HEIGHT  40
#define FREQ_SEL_COMMA_SIZE         10
...
void FrequencySelect::onPosChange() {
    ...
    int commaOffset = 0;
    int digitPosY = widgetPos.y;
    for (int i = 0; i < 12; i++) {
        digitTopMins[i] = ImVec2(widgetPos.x + (i * digitWidth) + commaOffset, digitPosY);
        digitBottomMins[i] = ImVec2(widgetPos.x + (i * digitWidth) + commaOffset, digitPosY + FREQ_SEL_DIGIT_HALF_HEIGHT);

        digitTopMaxs[i] = ImVec2(widgetPos.x + (i * digitWidth) + commaOffset + digitWidth, digitPosY + FREQ_SEL_DIGIT_HALF_HEIGHT);
        digitBottomMaxs[i] = ImVec2(widgetPos.x + (i * digitWidth) + commaOffset + digitWidth, digitPosY + FREQ_SEL_DIGIT_HEIGHT);

        if ((i + 1) % 3 == 0 && i < 11) {
            commaOffset += FREQ_SEL_COMMA_SIZE;
        }
    }
}
...
void FrequencySelect::draw() {
    ...
    ImGui::PushFont(style::digitFont);
    ImVec2 digitSz = ImGui::CalcTextSize("0");
    widgetPos.y = window->Pos.y + cursorPos.y - 10.0f;
    ...
    ImU32 offSegColor = ImGui::GetColorU32(ImGuiCol_Text, 0.1f);
    ...
    bool zeros = true;
    ImGui::ItemSize(ImRect(digitTopMins[0], ImVec2(digitBottomMaxs[11].x + 15, 60.0f)));
    ...
    for (int i = 0; i < 12; i++) {
        ...
        window->DrawList->AddText(ImVec2(widgetPos.x + (i * digitWidth) + commaOffset - 3, widgetPos.y),
                                  offSegColor, "8");
        window->DrawList->AddText(ImVec2(widgetPos.x + (i * digitWidth) + commaOffset - 3, widgetPos.y),
                                  zeros ? disabledColor : textColor, buf);
        if ((i + 1) % 3 == 0 && i < 11) {
            commaOffset += FREQ_SEL_COMMA_SIZE;
            window->DrawList->AddText(ImVec2(widgetPos.x + (i * digitWidth) + commaOffset + textOffset + 7, widgetPos.y),
