...
#define FREQ_SCALE_HEIGHT 		60
#define FREQ_SCALE_LINE_OFFSET 	10
#define WATERFALL_MOVING_SPEED	20
#define RESIZING_BAR_WIDTH		45
...
namespace ImGui {
    ...
    void WaterFall::drawFFT() {
        ...
        for (double freq = startFreq; freq < upperFreq; freq += range) {
            ...
            window->DrawList->AddLine(ImVec2(roundf(xPos), fftAreaMax.y),
                                      ImVec2(roundf(xPos), fftAreaMax.y + FREQ_SCALE_LINE_OFFSET),
                                      text, style::uiScale);
			window->DrawList->AddLine(ImVec2(roundf(xPos), fftAreaMax.y + FREQ_SCALE_HEIGHT),
                                      ImVec2(roundf(xPos), fftAreaMax.y + FREQ_SCALE_HEIGHT - FREQ_SCALE_LINE_OFFSET),
                                      text, style::uiScale);
            printAndScale(freq, buf);
            ImVec2 txtSz = ImGui::CalcTextSize(buf);
            window->DrawList->AddText(ImVec2(roundf(xPos - (txtSz.x / 2.0)), fftAreaMax.y + (FREQ_SCALE_HEIGHT - txtSz.y) / 2), text, buf);
        }
		
		// Resizing Bar
		ImVec2 resizeMin = ImVec2(widgetPos.x, fftAreaMax.y + 6);
		ImVec2 resizeMax = ImVec2(widgetPos.x + RESIZING_BAR_WIDTH, fftAreaMax.y + FREQ_SCALE_HEIGHT - 4);
		window->DrawList->AddRectFilled(resizeMin, resizeMax, GetColorU32(ImGuiCol_Button), 5, ImDrawFlags_RoundCornersTopRight | ImDrawFlags_RoundCornersBottomRight);
		ImU32 arrowColor = GetColorU32(ImGuiCol_CheckMark);
		ImVec2 p1 = ImVec2((resizeMin.x + resizeMax.x) / 2, resizeMin.y + 7);
		ImVec2 p2 = ImVec2(p1.x - 7, p1.y + 7);
		ImVec2 p3 = ImVec2(p1.x + 7, p1.y + 7);
		window->DrawList->AddTriangle(p1, p2, p3, arrowColor);
		window->DrawList->AddTriangleFilled(p1, p2, p3, arrowColor);
		p1 = ImVec2(p1.x, resizeMax.y - 7);
		p2 = ImVec2(p2.x, p1.y - 7);
		p3 = ImVec2(p3.x, p1.y - 7);
		window->DrawList->AddTriangle(p1, p2, p3, arrowColor);
		window->DrawList->AddTriangleFilled(p1, p2, p3, arrowColor);
    ...
    void WaterFall::drawWaterfall() {
		...
		// X Axis
        window->DrawList->AddLine(ImVec2(wfMin.x, wfMin.y),
                                  ImVec2(wfMax.x, wfMin.y),
                                  textColor, style::uiScale);
        // Y Axis
        window->DrawList->AddLine(ImVec2(wfMin.x, wfMin.y),
                                  ImVec2(wfMin.x, wfMax.y - 1),
                                  textColor, style::uiScale);

		char buf[32];
		int elapsed = 1;
		int yPos = wfMin.y + WATERFALL_MOVING_SPEED;
		int yEnd = wfMax.y - WATERFALL_MOVING_SPEED;
		float textVOffset = 10.0f * style::uiScale;
		while (yPos <= yEnd) {
			snprintf(buf, sizeof(buf), "%ds", elapsed);
			ImVec2 textSize = ImGui::CalcTextSize(buf);
			float textYPos = roundf(yPos - textSize.y / 2.0f);
			window->DrawList->AddText(ImVec2(wfMin.x - textSize.x - textVOffset, textYPos), textColor, buf);
			elapsed ++;
			yPos += WATERFALL_MOVING_SPEED;
		}
    }
    ...
    void WaterFall::processInputs() {
		...
		mouseInFFTResize = (dragOrigin.x > widgetPos.x && dragOrigin.x < widgetPos.x + RESIZING_BAR_WIDTH && dragOrigin.y >= freqAreaMin.y && dragOrigin.y <= freqAreaMax.y);
        ...
        if (fftResizeSelect) {
            ImGui::SetMouseCursor(ImGuiMouseCursor_ResizeNS);
			float barCenterY = (freqAreaMin.y + freqAreaMax.y) / 2;
			float deltaY = mousePos.y - barCenterY;
			newFFTAreaHeight = FFTAreaHeight + deltaY;
            newFFTAreaHeight = std::clamp<float>(newFFTAreaHeight, 150, widgetSize.y - 50);
			ImGui::GetForegroundDrawList()->AddRectFilled(ImVec2(widgetPos.x, widgetPos.y + newFFTAreaHeight - FREQ_SCALE_HEIGHT), ImVec2(widgetEndPos.x, widgetPos.y + newFFTAreaHeight), IM_COL32(255, 255, 255, 90));										
            return;
        }
    ...
    void WaterFall::onResize() {
        ...
        if (waterfallVisible) {
			FFTAreaHeight = std::min<int>(FFTAreaHeight, widgetSize.y - 70.0f);
            newFFTAreaHeight = FFTAreaHeight;
			fftHeight = FFTAreaHeight - 70.0f;
			waterfallHeight = widgetSize.y - fftHeight - 70.0f - 2;
        }
        else {
			fftHeight = widgetSize.y - 70.0f;
        }
        ...
        freqAreaMax = ImVec2(fftAreaMax.x, fftAreaMax.y + (FREQ_SCALE_HEIGHT * style::uiScale));
    ...
    double WaterFall::getLowerFrequency() {
        return lowerFreq;
    }
    
    double WaterFall::getUpperFrequency() {
        return upperFreq;
    }
