...
namespace sourcemenu {
    ...
    void updateOffset() {
        ...
        else if (offsetMode == OFFSET_MODE_VU_GPSDR) {
            effectiveOffset = UP_CONVERTER_TUNING_OFFSET;
        } // VU GPSDR onboard Up-conversion
    ...
    void draw(void* ctx) {
        ...
        // Update offsetMode according to Up-Converter's state
        if (core::upConverter.isEnabled()) {
            if (offsetMode == OFFSET_MODE_NONE) {
                offsetMode = OFFSET_MODE_VU_GPSDR;
                updateOffset();
                core::configManager.acquire();
                core::configManager.conf["offsetMode"] = offsetMode;
                core::configManager.release(true);
            }
        } else if (offsetMode != OFFSET_MODE_NONE) {
            offsetMode = OFFSET_MODE_NONE;
            updateOffset();
            core::configManager.acquire();
            core::configManager.conf["offsetMode"] = offsetMode;
            core::configManager.release(true);
